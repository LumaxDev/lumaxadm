#!/bin/bash
# ============================================================ #
# ==        Установщик фреймворка «LumaxADM» v1.0             == #
# ============================================================ #
#
...
echo -e "${C_CYAN}[i] Запускаю загрузчик LumaxADM...${C_RESET}"

# --- Скачивание архива ---
REPO_ARCHIVE_URL="https://github.com/${REPO_OWNER}/${REPO_NAME}/archive/refs/heads/${REPO_BRANCH}.tar.gz"
TEMP_DIR=$(mktemp -d /tmp/lumaxadm_bootstrap.XXXXXX)

echo -e "${C_CYAN}[i] Скачиваю последнюю версию из ветки '${REPO_BRANCH}'...${C_RESET}"
if ! curl -sL --fail -o "${TEMP_DIR}/lumaxadm.tar.gz" "$REPO_ARCHIVE_URL"; then
...
if ! tar -xzf "${TEMP_DIR}/lumaxadm.tar.gz" -C "$TEMP_DIR" --strip-components=1; then
...
INSTALL_SCRIPT="${TEMP_DIR}/lumaxadm.sh"
if [[ -f "$INSTALL_SCRIPT" ]]; then
    ...
    echo -e "${C_RED}[✗] Критическая ошибка: главный файл lumaxadm.sh не найден в архиве!${C_RESET}"
    rm -rf "$TEMP_DIR"
    exit 1
fi
