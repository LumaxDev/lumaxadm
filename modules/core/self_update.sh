...
# ============================================================ #
#
# Управляет жизненным циклом самого "LumaxADM".
#
...
_perform_install_or_update() {
    ...
    local TEMP_ARCHIVE; TEMP_ARCHIVE=$(mktemp /tmp/lumaxadm_archive.XXXXXX.tar.gz)
    
    printf_info "Качаю последнюю версию с GitHub..."
    if ! curl -sL --fail -o "$TEMP_ARCHIVE" "$REPO_ARCHIVE_URL"; then
        printf_error "Не могу скачать архив. Проверь интернет или настройки в config/lumaxadm.conf"
        rm -f "$TEMP_ARCHIVE"; return 1
    fi

    local TEMP_DIR; TEMP_DIR=$(mktemp -d /tmp/lumaxadm_extracted.XXXXXX)
...
install_script() {
    info "Запуск процедуры установки LumaxADM (локальная копия из SCRIPT_DIR)..."
    ...
    info "Создаю системную команду 'lumaxadm' (через symlink на /opt/lumaxadm/lumaxadm.sh)..."
    run_cmd ln -sf "${INSTALL_DIR}/lumaxadm.sh" "$INSTALL_PATH"
    run_cmd chmod +x "${INSTALL_DIR}/lumaxadm.sh"
    
    # Добавляем алиас для root, если он нужен
    if ! grep -q "alias lumaxadm='sudo lumaxadm'" /root/.bashrc 2>/dev/null; then
        echo "alias lumaxadm='sudo lumaxadm'" | run_cmd tee -a /root/.bashrc >/dev/null
    fi

    log "Скрипт успешно установлен."
    ok "LumaxADM установлен в системе."
    printf "   %b: %b\n" "${C_BOLD}Команда запуска" "${C_YELLOW}lumaxadm${C_RESET}"

    # --- Умный враппер: работает с sudo и без него ---
    local wrapper_script="/usr/local/bin/lumaxadm"
    # Удаляем старый симлинк, создаём умный wrapper-скрипт
    rm -f "${wrapper_script}"
    cat > "${wrapper_script}" << 'WRAPPER_EOF'
#!/bin/bash
# Умный лаунчер LumaxADM: пробует sudo, если нет — запускает напрямую
TARGET="/opt/lumaxadm/lumaxadm.sh"
if [[ $EUID -eq 0 ]]; then
    exec "$TARGET" "$@"
...
WRAPPER_EOF
    ...
    # Убираем устаревший алиас sudo lumaxadm из .bashrc (если есть)
    if [ -f /root/.bashrc ]; then
        sed -i "/alias lumaxadm=/d" /root/.bashrc 2>/dev/null || true
    fi

    warn "ВАЖНО: переподключись к серверу, чтобы команда заработала в новой сессии."

    # Автозапуск LumaxADM сразу после установки
    if [[ "${LUMAXADM_NO_AUTOSTART:-0}" != "1" ]]; then
        echo ""
        info "Стартую LumaxADM прямо сейчас..."
        sleep 1
        exec "$wrapper_script"
    fi
}
...
uninstall_script() {
    warn "Точно хочешь выгнать LumaxADM НАХУЙ? УДАЛЮТСЯ ВСЕ ЕЁ ФАЙЛЫ!"
    if ! ask_yes_no "(y/n): " "n"; then info "Отмена удаления. LumaxADM остаётся."; return; fi
    info "Начинаю самоликвидацию (удаляю бинарь, каталог /opt/lumaxadm, лог и базу флота)..."
    run_cmd rm -f "$INSTALL_PATH"
    run_cmd rm -rf "/opt/lumaxadm"
    ...
    if [ -f "/root/.bashrc" ]; then run_cmd sed -i "/alias lumaxadm='sudo lumaxadm'/d" /root/.bashrc; fi
...
check_for_updates() {
    local remote_version_url="https://raw.githubusercontent.com/${REPO_OWNER}/${REPO_NAME}/${REPO_BRANCH}/lumaxadm.sh?cb=$(date +%s)"
    local remote_ver
...
run_update() {
    if _perform_install_or_update "update"; then
        ...
        info "Перезапускаю LumaxADM, чтобы все модули подхватили новую версию..."
        sleep 2
        exec "$INSTALL_PATH"
    fi
}