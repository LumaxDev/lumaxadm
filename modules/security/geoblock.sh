...
GLOBAL_WHITELIST_FILE="/etc/lumaxadm/global-whitelist.txt"
...
_geo_get_cron_schedule() {
    local cron_file="/etc/cron.d/lumaxadm-geoblock-update"
    if [[ -f "$cron_file" ]]; then
        local line
...
_geo_show_status() {
    ...
    if run_cmd systemctl is-enabled lumaxadm-geoblock &>/dev/null 2>&1; then
        autostart="${C_GREEN}Включена${C_RESET}"
    fi

    local autoupdate="${C_GRAY}Выключено${C_RESET}"
    if [[ -f "/etc/cron.d/lumaxadm-geoblock-update" ]]; then
        local sched
...
_geo_activate() {
    ...
    local temp_wl_ipset="lumaxadm_geo_whitelist_temp"
    ...
    if ! command -v global_whitelist_prepend_system_ips &>/dev/null; then
        if [[ -f "${SCRIPT_DIR:-/opt/lumaxadm}/modules/security/whitelist_manager.sh" ]]; then
            source "${SCRIPT_DIR:-/opt/lumaxadm}/modules/security/whitelist_manager.sh" && found_wl_manager=1
    ...
    # Swap whitelist
    if ! run_cmd ipset list lumaxadm_geo_whitelist &>/dev/null; then
        run_cmd ipset create lumaxadm_geo_whitelist hash:net hashsize 256 maxelem 1024 2>/dev/null || true
    fi
    run_cmd ipset swap lumaxadm_geo_whitelist "$temp_wl_ipset"
    run_cmd ipset destroy "$temp_wl_ipset" 2>/dev/null || true
    ...
    run_cmd ipset save lumaxadm_geo_whitelist | run_cmd tee "$GEO_CONFIG_DIR/lumaxadm_geo_whitelist.ipset" > /dev/null
...
-A ufw-before-input ! -i lo -m set --match-set ${GEO_IPSET_NAME} src -m set ! --match-set lumaxadm_geo_whitelist src -m conntrack --ctstate NEW -j DROP
...
#!/bin/bash
# LumaxADM Geo-Block: Быстрое оффлайн-восстановление ipset после ребута
...
if [[ -f "${GEO_CONFIG_DIR}/${GEO_IPSET_NAME}.ipset" && -f "${GEO_CONFIG_DIR}/lumaxadm_geo_whitelist.ipset" ]]; then
    echo "[i] Восстановление Geo-Block из локального кэша..."
    if ipset restore < "${GEO_CONFIG_DIR}/${GEO_IPSET_NAME}.ipset" && \
       ipset restore < "${GEO_CONFIG_DIR}/lumaxadm_geo_whitelist.ipset"; then
...
ipset create lumaxadm_geo_whitelist hash:net hashsize 256 maxelem 1024 2>/dev/null || true
...
if [[ -f "${GLOBAL_WHITELIST_FILE}" ]]; then
    ...
    grep -v '^\s*#' "${GLOBAL_WHITELIST_FILE}" | grep -v '^\s*$' | awk '{print $1}' | while read -r ip; do
        ip="${ip//[$'\r\n\t ']/}"
        if [[ -n "$ip" && "$ip" != *:* ]]; then
            echo "add lumaxadm_geo_whitelist $ip -exist" >> "$TEMP_WL_RESTORE"
        fi
    done
...
_geo_test_ip() {
    ...
    if run_cmd ipset list lumaxadm_geo_whitelist &>/dev/null; then
        is_active=1
        if ipset test lumaxadm_geo_whitelist "$ip" &>/dev/null; then
...
_geo_write_updater_script() {
    local updater_script="/usr/local/bin/lumaxadm-geoblock-update.sh"
    
    cat <<'UPDATE_SCRIPT' | run_cmd tee "$updater_script" > /dev/null
#!/bin/bash
# LumaxADM Geo-Block: Тихое фоновое автообновление правил
export SCRIPT_DIR="/opt/lumaxadm"
source "${SCRIPT_DIR}/modules/core/common.sh" 2>/dev/null || true
source "${SCRIPT_DIR}/modules/security/geoblock.sh" 2>/dev/null || true

# Временный лог
exec 1>>/var/log/lumaxadm-geoblock-update.log 2>&1

echo "[$(date '+%Y-%m-%d %H:%M:%S')] Запуск автообновления..."

if [[ ! -f "/etc/lumaxadm/geoblock/countries.txt" ]] || [[ ! -s "/etc/lumaxadm/geoblock/countries.txt" ]]; then
...
while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    line="${line^^}"
    [[ -n "$line" ]] && countries+=("$line")
done < "/etc/lumaxadm/geoblock/countries.txt"

total_countries=${#countries[@]}
temp_dir=$(mktemp -d)
temp_ipset="lumaxadm_geoblock_temp"
...
if [[ "$subnets_total" -gt 0 && -f "$temp_dir/restore.txt" ]]; then
    if ipset restore < "$temp_dir/restore.txt"; then
        # Swap
        if ! run_cmd ipset list lumaxadm_geoblock &>/dev/null; then
            ipset create lumaxadm_geoblock hash:net hashsize 65536 maxelem 500000
        fi
        ipset swap lumaxadm_geoblock "$temp_ipset"
        ipset destroy "$temp_ipset" 2>/dev/null || true
        
        # Сохраняем в кэш
        mkdir -p "/etc/lumaxadm/geoblock"
        ipset save lumaxadm_geoblock > "/etc/lumaxadm/geoblock/lumaxadm_geoblock.ipset"
