    ...
    for ip_key in "${ordered_keys[@]}"; do
        ...
        if [[ "$first_entry" -eq 1 ]]; then
            if ! echo "$current_content" | grep -q "СИСТЕМНЫЕ IP — НЕ УДАЛЯТЬ"; then
                new_block+="# ─────────────────────────────────────────────────────────────────
# ⚠️  СИСТЕМНЫЕ IP — НЕ УДАЛЯТЬ! Добавлено автоматически LumaxADM.
...
_gwl_sync_fail2ban() {
    ...
    run_cmd cp -f "$GLOBAL_WHITELIST_FILE" "/etc/lumaxadm/fail2ban-whitelist.txt" 2>/dev/null || true
...
# Добавляет whitelist IP в блок LumaxADM Anti-DDoS если он уже настроен
_gwl_sync_ufw() {
    local ips=("$@")
    local before_rules="/etc/ufw/before.rules"
    local antiddos_start="# --- НАЧАЛО: LumaxADM Anti-DDoS ---"
...
with open('/etc/ufw/before.rules', 'r') as f:
    content = f.read()
target = '# --- НАЧАЛО: LumaxADM Anti-DDoS ---'
...
show_global_whitelist_menu() {
    ...
    while true; do
        ...
        # Geo-block
        if command -v ipset &>/dev/null && ipset list lumaxadm_geo_whitelist -terse &>/dev/null; then
