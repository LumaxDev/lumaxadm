...
readonly TL_CONFIG_DIR="/etc/lumaxadm/traffic_limiter"
readonly TL_BPF_SRC_PATH="${SCRIPT_DIR}/modules/local/shaper.bpf.c"
readonly TL_BPF_OBJ_PATH="${TL_CONFIG_DIR}/shaper.bpf.o"
readonly TL_CTRL_PY_PATH="${SCRIPT_DIR}/modules/local/lumaxadm_ctrl.py"
readonly TL_SERVICE_NAME="lumaxadm-traffic-limiter.service"
readonly TL_SERVICE_PATH="/etc/systemd/system/${TL_SERVICE_NAME}"
readonly TL_OLD_APPLY_SCRIPT="/usr/local/bin/lumaxadm-traffic-limiter-apply.sh"
readonly TL_BPF_PIN_DIR="/sys/fs/bpf/lumaxadm"
...
_tl_show_shaper_intro() {
    echo -e "  ${C_CYAN}╔══════════════════════════════════════════════════════════╗${C_RESET}"
    echo -e "  ${C_CYAN}║${C_RESET}  ${C_YELLOW}⚡ LumaxADM eBPF Traffic Shaper v3.2 (HighLoad)${C_RESET}"
    echo -e "  ${C_CYAN}╠══════════════════════════════════════════════════════════╣${C_RESET}"
...
_tl_generate_ebpf_service_file() {
    ...
    cat <<EOF
[Unit]
Description=LumaxADM eBPF Traffic Limiter (Multi-Rule)
