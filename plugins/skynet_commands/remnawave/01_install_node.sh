...
setup_tls_acme_remote() {
    ...
    if command -v ufw >/dev/null 2>&1; then
        ufw allow 80/tcp comment 'lumaxadm remnanode acme http-01' >/dev/null 2>&1 || true
    fi
...
main() {
    ...
    if [[ -z "${SELFSTEAL_DOMAIN:-}" ]]; then
        log_error "SELFSTEAL_DOMAIN не задан. Плагин должен вызываться только из LumaxADM/Skynet с нужным окружением."
        exit 1
    fi
